 ...
 * through apache commons logging 1.1.
 * 
 * History :
 * 
 * - 05/05/2009 : Updated API to EclipseLink 1.1 version + Javadoc fixes - 08/05/2009 : Fix for log4j levels in conflict
 * with eclipselink jpa levels (shouldLog)
 * 
 * TODO : Use Enum instead of int values for Levels (OO Design)
 * 
 * Requires : - Jakarta Commons Logging 1.1 : http://commons.apache.org/logging/ - commons-logging-1.1.1.jar
 * 
 * - Log4j 1.2 : http://logging.apache.org/log4j/ - log4j-1.2.15.jar
 * 
 * 
 * @author laurent bourges (voparis) : bourges.laurent@gmail.com
 * 
 * @see org.eclipse.persistence.logging.AbstractSessionLog
 * @see org.eclipse.persistence.logging.JavaLog
...
public final class CommonsLoggingSessionLog extends AbstractSessionLog {
	...
	/**
	 * Stores the default session name in case there is the session name is missing. org.eclipse.persistence package
	 * used by Log4J configuration
	 */
	 ...
	 */
	public static final Level[] JAVA_LEVELS = new Level[] { Level.ALL, Level.FINEST, Level.FINER, Level.FINE,
	 ...
	 * (clone does not deep clone this instance)
	 */
	...
	/**
	 * LogWrapper instances. Acts as a static variable but not declared static to avoid classLoader leaks (clone does
	 * not deep clone this map) Note : Unsynchronized Map = should be thread-safe !
	 */
	 ...
	 *
	 * Used by : org.eclipse.persistence.internal.jpa.EntityManagerSetupImpl.updateLoggers : creates an instance for
	 * singletonLog and sessionLog
	 *
	...
	public static final void onInit() {
		...
		if (FORCE_APACHE_COMMONS_LOGGING_DIAGNOSTICS) {
			/**
			 * The name (<code>org.apache.commons.logging.diagnostics.dest</code>) of the property used to enable
			 * internal commons-logging diagnostic output, in order to get information on what logging implementations
			 * are being discovered, what classloaders they are loaded through, etc.
			 * <p>
			 * If a system property of this name is set then the value is assumed to be the name of a file. The special
			 * strings STDOUT or STDERR (case-sensitive) indicate output to System.out and System.err respectively.
			 * <p>
			 * Diagnostic logging should be used only to debug problematic configurations and should not be set in
	...
	/**
	 * PUBLIC: Return the effective log level for the name space extracted from session and category. If a Logger's
	 * level is set to be null then the Logger will use an effective Level that will be obtained by walking up the
	 * parent tree and using the first non-null Level.
	 *
	 * @param category
	 *            category
	 *
	...
	public final int getLevel(final String category) {
		...
		if (FORCE_INTERNAL_DEBUG) {
			debug("CommonsLoggingSessionLog.getLevel : OUT : category : " + category + " : level : "
					+ getLevelString(l));
		}
	 ...
	 *
	 * @param level
	 *            value according to the java.util.logging.Levels
	 * @param category
	 *            category
	 */
	@Override
		...
		AccessController.doPrivileged(new PrivilegedAction<Object>() {
			...
			public Object run() {
				...
				if (lw == null) {
					...
					if (logger == null) {
						error("CommonsLoggingSessionLog.setLevel : Logger not found : " + category + " : "
								+ lw.getLog());
					} else {
						...
						if (realLevel == org.apache.log4j.Level.OFF) {
							// force level to OFF :
							lw.setLevel(OFF);
							error("CommonsLoggingSessionLog.setLevel : unknown level : " + getLevelString(level)
									+ " : level set to OFF");
						} else {
							lw.setLevel(level);
						}
						...
						if (logger.isEnabledFor(org.apache.log4j.Level.WARN)) {
							logger.warn("CommonsLoggingSessionLog.setLevel : Logger Level set to : "
									+ logger.getLevel());
						}
	...
	/**
	 * PUBLIC: Check if a message of the given lev would actually be logged by the logger with name space built from the
	 * given session and category. Return the shouldLog for the given category Note : this method is very very used so
	 ...
	 *
	 * @param entry
	 *            SessionLogEntry that holds all the information for a EclipseLink logging event
	 */
	public final void log(final SessionLogEntry entry) {
		if (shouldLog(entry.getLevel(), entry.getNameSpace())) {
			if (FORCE_INTERNAL_DEBUG) {
				debug("CommonsLoggingSessionLog.log : namespace : " + entry.getNameSpace() + " message : "
						+ entry.getMessage());
			}
	...
	private final void addLogger(final String loggerCategory, final String loggerNameSpace) {
		if (FORCE_INTERNAL_DEBUG) {
			debug("CommonsLoggingSessionLog.addLogger : category : " + loggerCategory + " in name space : "
					+ loggerNameSpace);
		}

		this.CATEGORY_LOGGERS.put(loggerCategory,
				new LogWrapper(this, loggerCategory, LogFactory.getLog(loggerNameSpace)));
	}
	...
	/**
	 * INTERNAL: Return the LogWrapper instance for the given category Note : this method is very very used so optimized
	 * for performance.
	 *
	 * @param category
	 *            category
	 ...
	 *            SessionLogEntry that holds all the information for a EclipseLink logging event
	 * @param level
	 *            according to eclipselink level
	 * @param log
	 *            commons-logging 1.1 wrapper
	 */
	 ...
	 *
	 * @see org.eclipse.persistence.logging.AbstractSessionLog#getLevelString() Return the log level as a string value.
	 */
	 ...
	 *            message to print
	 * @param printStack
	 *            adds a stack trace to find caller class
	 */
	...
	private static final class LogWrapper {
		...
		protected final void addChild(final LogWrapper lw) {
			...
			if (FORCE_INTERNAL_DEBUG) {
				System.out.println("CommonsLoggingSessionLog.LogWrapper.addChild : this : " + getCategory()
						+ " : child : " + lw.getCategory());
			}
		}
		...
		private final int computeLevel(final int localLevel) {
			...
			if (FORCE_INTERNAL_DEBUG) {
				System.out.println("CommonsLoggingSessionLog.LogWrapper.computeLevel : IN : " + getCategory()
						+ " : level : " + CommonsLoggingSessionLog.getLevelString(localLevel));
			}

			while ((lw != null)
					&& (((lw == this) && (localLevel == UNDEFINED_LEVEL)) || ((lw != this) && (lw.getLevel() == UNDEFINED_LEVEL)))) {
				pw = lw.parent;
			...
			if (lw != null) {
				...
				if (FORCE_INTERNAL_DEBUG) {
					System.out.println("CommonsLoggingSessionLog.LogWrapper.computeLevel : category : "
							+ lw.getCategory() + " - computedLevel : " + getLevelString(computedLevel) + " : "
							+ lw.getLog());
				}
				final Logger logger = getLog4JLogger(lw.getLog());

				if (logger == null) {
					error("CommonsLoggingSessionLog.computeLevel : Logger not found : " + lw.getCategory() + " : "
							+ lw.getLog());
				} else {
					...
					if (FORCE_INTERNAL_DEBUG) {
						System.out.println("CommonsLoggingSessionLog.LogWrapper.computeLevel : category : "
								+ lw.getCategory() + " :  realLevel : " + realLevel + " - enabled = " + enabled);
					}
				}
			...
			if (FORCE_INTERNAL_DEBUG) {
				System.out.println("CommonsLoggingSessionLog.LogWrapper.computeLevel : OUT : " + getCategory()
						+ " : level : " + CommonsLoggingSessionLog.getLevelString(computedLevel));
			}
