...
/**
 * This code is mostly from http://balusc.blogspot.com/2013/01/composite-component -with-multiple-input.html.
 * 
...
public class InputDate extends UIInput implements NamingContainer {
	...
	private static final String BASE_YEAR = "baseYear";
	// Fields
	// -------------------------------------------------------------------------------------
	...
	public void encodeBegin(FacesContext context) throws IOException {
		...
		int maxYear = makeAbsolute(calendar.get(Calendar.YEAR), getAttributeValue(MAX_YEAR, "+5"));
		int minYear = makeAbsolute(calendar.get(Calendar.YEAR), getAttributeValue(MIN_YEAR, "-5"));
		...
		if (date != null) {
			...
			if (year > maxYear || minYear > year) {
				throw new IllegalArgumentException(String.format("Year %d out of min/max range %d/%d.", year, minYear,
						maxYear));
			}
		}
		...
		year.setValue(calendar.get(Calendar.YEAR) - baseYear);
		...
		setYears(createIntegerArray(maxYear - baseYear, minYear - baseYear));
		super.encodeBegin(context);
	}

	private int makeAbsolute(int currentYear, String v) {
		if (v.startsWith("+")) {
			return currentYear + Integer.parseInt(v.substring(1).trim());
	...
	/**
	 * Returns the submitted value in LocaleDate format.
	 */
	@Override
	public Object getSubmittedValue() {
		// if one of them is null, result is null
		if (year.getValue() == null || month.getValue() == null || day.getValue() == null)
			return null;
		int baseYear = getBaseYear();
		return new LocaleDate(baseYear + (Integer) year.getValue(), ((Integer) month.getValue()).byteValue(),
				((Integer) day.getValue()).byteValue());
	}
	...
	protected Object getConvertedValue(FacesContext context, Object submittedValue) {
		return ((LocaleDate) submittedValue).getDate();
	}
	...
	public void updateDaysIfNecessary(AjaxBehaviorEvent event) {
		...
		com.ibm.icu.util.Calendar calendar = Locale.getInstance().getCalendar();
		calendar.set(Calendar.MONTH, (Integer) month.getValue() - 1);
		calendar.set(Calendar.YEAR, baseYear + (Integer) year.getValue());
		...
		if (getDays().length != maxDay) {
			...
			if ((Integer) day.getValue() > maxDay) {
				day.setValue(maxDay); // Fix the selected value if it exceeds
										// new max value.
			}

			FacesContext context = FacesContext.getCurrentInstance(); // Update
																		// day
																		// field.
	...
	private Integer getBaseYear() {
		return getAttributeValue(BASE_YEAR, 0);
	}
